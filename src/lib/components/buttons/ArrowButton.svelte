...
<script lang="ts">
  ...
  const directionConfig = $derived.by(() => {
    switch (direction) {
      case "right":
        return { icon: "caret-right", label: "Freccia destra" };
      case "up":
        return { icon: "caret-up", label: "Freccia su" };
      case "down":
        return { icon: "caret-down", label: "Freccia giù" };
      case "left":
      default:
        return { icon: "caret-left", label: "Freccia sinistra" };
    }
  });
