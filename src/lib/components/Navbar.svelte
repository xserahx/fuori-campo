<!--Componente navbar con 2 versioni: desktop e mobile (hamburger menu e tendina a comparsa)-->
...
<script lang="ts">
  ...
  $effect(() => {
    ...
    const onPointerMove = (e: PointerEvent) => {
      ...
      if (e.clientY <= 80 && (e.clientY < lastY || e.movementY < 0))
        visible = true;
      lastY = e.clientY;
    };
...
>
  <div class="navbar-inner">
    ...
    <nav class="nav-links" aria-label="Navigazione principale">
      {#each navItems as item, i}
        <a class="nav-link" class:is-active={i === activeIndex} href={item.href}
          >{item.label}</a
        >
      {/each}
    </nav>
...
<style>
  ...
  .nav-burger.is-open .bar-top {
    transform: translateY(8.96px) rotate(45deg);
  }
  .nav-burger.is-open .bar-mid {
    opacity: 0;
    transform: scaleX(0.3);
  }
  .nav-burger.is-open .bar-bot {
    transform: translateY(-8.96px) rotate(-45deg);
  }
  ...
  .mobile-nav-link {
    ...
    opacity: 0;
    animation: link-in 420ms var(--ease-cinema) calc(60ms + var(--i, 0) * 70ms)
      forwards;
    transition:
