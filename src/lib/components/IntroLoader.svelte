<!--Componente per il loader dell'introduzione con testo, percentuale di caricamento e immagini di volontari che scorrono-->
<script lang="ts">
  ...
  const IMAGES = (() => {
    const vols = getCachedVolunteers().filter(
      (v) =>
        v.ha_immagini && ((v.image_paths?.length ?? 0) > 0 || v.image_path),
    );
    const out: string[] = [];
  ...
  onMount(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      gsap
        ...
        .from(
          ".img-strip img",
          { duration: 0.8, y: 150, opacity: 0, stagger: { amount: 1 } },
          "-=0.1",
        )
        .from(
          ".bottom-line",
          { duration: 0.5, y: 50, opacity: 0, stagger: 0.1 },
          "-=0.5",
        );
    }, shellEl);
    return () => ctx.revert();
      ...
      <div class="images">
        ...
        <div class="bottom-section">
          <span class="bottom-line"
            >Le Olimpiadi degli Invisibili · Milano Cortina 2026</span
          >
